        ...
        private void GetColumnTypes()
        {
            ...
            foreach (DataRow r in stats.Rows) {
                ...
                if (!((colDefault is DBNull) || (colDefault.ToString() == String.Empty)) && col.DefaultValue == DBNull.Value)
                {
                    ...
                    // similiar to StatsMySql save for the condition on CURRENT_TIMESTAMP - this could probably be expressed in various ways fro the MSSQL.
                    // The result will be though the same, just one additional call to refrection when trying to parse the funcion signature into a datetime.
